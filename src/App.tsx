 ...
 * Loads only after first user interaction
 ******************************/
function AnalyticsDeferred({ id }: { id: string }) {
  useEffect(() => {
    ...
    const load = () => {
      ...
      ['pointerdown','keydown','scroll'].forEach((ev) => window.removeEventListener(ev, load as any, {passive:true} as any));
    };
    ['pointerdown','keydown','scroll'].forEach((ev) => window.addEventListener(ev, load, { once: true, passive: true } as any));
    return () => ['pointerdown','keydown','scroll'].forEach((ev) => window.removeEventListener(ev, load as any));
  }, [id]);
  return null;
}
...
const base: React.SVGProps<SVGSVGElement> = {
  ...
  strokeLinejoin: "round",
    ...
    const og = (prop: string, content: string) => {
      const m = ensure(`meta[property="${prop}"]`, () => { const x = document.createElement('meta'); x.setAttribute('property', prop); return x; }) as HTMLMetaElement;
    ...
    const tw = (name: string, content: string) => {
      const m = ensure(`meta[name="${name}"]`, () => { const x = document.createElement('meta'); x.setAttribute('name', name); return x; }) as HTMLMetaElement;
      m.setAttribute('content', content);
    };
...
/******************************
 * Thank You page (route: /thank-you)
 ******************************/
  ...
  return (
    <div className="animated-bg min-h-screen w-full text-slate-900 grid place-items-center px-4">
      ...
      <div className="max-w-xl text-center space-y-6 fade-section is-visible">
        ...
        <p className="text-slate-600">We’ll review your message and files and reply within 24 hours at
          <span className="font-medium"> custombuildstudio@gmail.com</span> or by phone.
        </p>
        <a href="/">
          <Button className="rounded-2xl">Back to Home</Button>
        </a>
      </div>
    </div>
  );
...
export default function Site() {
  ...
  return (
    <div className="animated-bg min-h-screen w-full text-slate-900">
      {/* Self-test: subject field exists for Netlify emails */}
      <script dangerouslySetInnerHTML={{__html: `try{console.assert(!!document.querySelector('form[name=\"contact\"] input[name=\"subject\"]'), 'subject hidden field should exist');}catch(e){}`}} />
      <GlobalAnimatedBackground />
      <HeadTags
      ...
      <section className="fade-section mx-auto max-w-6xl px-4 py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            <a href="#contact"><Button size="lg" className="rounded-2xl bg-sky-500 hover:bg-sky-600 text-white transition-colors">Start Your Project</Button></a>
            <a href="#work"><Button size="lg" variant="outline" className="rounded-2xl border-slate-300 text-slate-800 hover:bg-slate-100 transition-colors">See Examples</Button></a>
          </div>
        ...
        <div className="grid gap-4">
          <Card className="rounded-2xl shadow-md"><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Printer className="h-4 w-4"/> 3D Printing</CardTitle></CardHeader><CardContent className="text-sm text-slate-600">FDM & Resin. Enclosed chamber available for ABS/ASA. Bed slinger + CoreXY. Prusa MK4S, Neptune 4 Pro, CR-10 Pro.</CardContent></Card>
          <Card className="rounded-2xl shadow-md"><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Cube className="h-4 w-4"/> CAD & Reverse-Engineering</CardTitle></CardHeader><CardContent className="text-sm text-slate-600">Fusion 360 & Onshape. From sketch to solid models, organic shapes, and fit-for-purpose redesigns.</CardContent></Card>
          <Card className="rounded-2xl shadow-md"><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Cog className="h-4 w-4"/> CNC Programming</CardTitle></CardHeader><CardContent className="text-sm text-slate-600">Milling & lathe toolpaths with shop-ready posts. Experience with Siemens, Mazak, Haas, DMG, and more.</CardContent></Card>
        </div>
      </section>
      ...
      <section id="work" className="fade-section mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-8">
          {["Gauge pod for automotive dash (PETG)", "ABS enclosure w/ heat-set inserts", "PETG replacement for IKEA table part"].map((title, i) => (
      ...
      <section id="pricing" className="fade-section mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-8">
          {[{ name: "3D Printing", price: "From $0.20/gram + setup", items: ["Material: PLA, PETG, ABS/ASA", "Rush options available", "Bulk discounts"] }, { name: "CAD & Reverse-Eng", price: "$35–$55/hr (scope-based)", items: ["Simple parts often fixed-fee", "Includes DFM notes", "Export: STEP/STL"] }, { name: "CNC Programming", price: "$45–$75/hr (machine-dependent)", items: ["Setup sheets included", "Tool lists + posts", "Remote or onsite"] }].map((p) => (
      ...
      <section className="fade-section mx-auto max-w-6xl px-4 py-12">
        <div className="grid md:grid-cols-3 gap-6">
          {[{title: "Shop-Floor Practical", desc: "Advice from a working CNC machinist—designs that print and parts that fit."}, {title: "Clear Communication", desc: "One owner-operator point of contact. You’ll always know status and ETA."}, {title: "Local & Reliable", desc: "Pickup in Edmonton, or ship anywhere in Canada."}].map((t) => (
      ...
      <section id="contact" className="fade-section mx-auto max-w-6xl px-4 py-12">
        <div className="grid md:grid-cols-2 gap-10 items-start">
          ...
          <Card className="rounded-2xl p-6">
            <form method="POST" data-netlify="true" name="contact" encType="multipart/form-data" netlify-honeypot="bot-field" action="/thank-you" className="space-y-4" onSubmit={(e) => {
